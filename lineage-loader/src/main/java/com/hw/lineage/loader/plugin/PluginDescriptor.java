 ...
 *
 * <p>PluginDescriptor 主要包含三个关键信息：
 * 1. 插件的唯一标识符（pluginId）
 * 2. 插件所需加载资源的 URL 数组（通常是 JAR 文件的 URL）
...
public class PluginDescriptor {
    ...
    /**
     * 指向该插件相关资源（例如 JAR 包）的 URL 数组。
     * 当插件类加载器加载该插件时，会在这些 URL 中查找插件所需的类和资源。
     * 一般包含插件的主 JAR 文件，以及该插件所依赖的其他 JAR 文件 URL。
     */
     ...
     * 或者直接排除插件本身对这些类的加载。
     *
     * 与 ChildFirstClassLoader 中的 alwaysParentFirstPatterns 类似，这里提供一个机制来避免插件内部的类与系统类冲突，
     ...
     * @param pluginResourceURLs 插件资源 URL 数组（指向该插件的所有相关 JAR 文件和资源文件）
     ...
     * 获取插件资源 URL 数组。
     *
     * @return 包含该插件所有相关资源（通常是 JAR 文件）的 URL 数组
     */
    ...
    /**
     * 返回该 PluginDescriptor 的字符串表示，包括插件 ID、资源 URL 列表和排除模式列表，便于日志与调试。
     *
     * @return 描述该插件的字符串信息
     */
    @Override
